...
export default function BudgetPageClient({ budgets, categories }: { budgets: BudgetWithSpent[]; categories: Category[] }) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">{getMonthName(month)} {year} — {formatCurrency(totalSpent)} / {formatCurrency(totalBudget)}</p>
        </div>
      ...
      {budgets.length > 0 && (
        <div className="rounded-2xl border border-slate-700/50 bg-slate-800/50 p-6">
          <div className="flex justify-between mb-2">
            <span className="text-sm text-white font-medium">Overall Budget Usage</span>
            <span className="text-sm text-slate-400">{totalBudget > 0 ? Math.round((totalSpent / totalBudget) * 100) : 0}%</span>
          </div>
      ...
      {budgets.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {budgets.map((budget) => (
            <div key={budget.id} className="group rounded-2xl border border-slate-700/50 bg-slate-800/50 p-5 hover:border-slate-600/50 transition-all">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <div className="w-3 h-3 rounded-full" style={{ backgroundColor: budget.categoryColor }} />
                  <span className="text-sm font-medium text-white">{budget.categoryName}</span>
                </div>
                <button onClick={() => handleDelete(budget.id)} className="opacity-0 group-hover:opacity-100 p-1.5 rounded-lg text-slate-400 hover:text-red-400 transition-all">
              ...
              <p className={`text-xs mt-2 ${budget.percentage > 100 ? 'text-red-400' : budget.percentage > 80 ? 'text-amber-400' : 'text-slate-500'}`}>
