...
function formatUserDetails(
  user: AdminCreateUserResult,
  appOrigin: string,
  copy: ReturnType<typeof useI18n>['messages']['pages']['admin']['createUser'],
  common: ReturnType<typeof useI18n>['messages']['pages']['admin']['common']
) {
  return [
    `${APP_NAME} account details`,
    '',
    `${copy.loginUrl}: ${appOrigin}/login`,
    `${copy.name}: ${user.name}`,
    `${copy.email}: ${user.email}`,
    `${common.role}: ${user.role === 'ADMIN' ? common.admin : common.user}`,
    `${common.package}: ${user.packageName || copy.noPackage}`,
    `${copy.temporaryPassword}: ${user.temporaryPassword}`,
    '',
    'First login requirement:',
    copy.firstLoginWarning,
  ].join('\n');
}
...
export default function AdminCreateUserPanel({ packages }: AdminCreateUserPanelProps) {
  ...
  const copy = adminCopy.createUser;
  ...
  const copyUserDetails = async () => {
    ...
    await navigator.clipboard.writeText(formatUserDetails(createdUser, appOrigin || window.location.origin, copy, adminCopy.common));
  ...
  return (
    <>
      ...
      <Modal isOpen={isOpen} onClose={() => setIsOpen(false)} title={copy.title} className="max-w-2xl">
        ...
        <form action={createUser} className="grid gap-4 sm:grid-cols-2">
          <Input id="createUserName" name="name" label={copy.name} placeholder={copy.namePlaceholder} required />
          <Input id="createUserEmail" name="email" label={copy.email} type="email" placeholder="user@example.com" required />
          <Select id="createUserRole" name="role" label={adminCopy.common.role} defaultValue="USER" options={roleOptions} />
          <Select id="createUserPackageId" name="packageId" label={adminCopy.common.package} defaultValue="" options={packageOptions} />
          ...
          <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700/50 dark:bg-slate-900/40">
            ...
            <div className="grid gap-2 rounded-xl border border-slate-200 bg-white p-3 text-sm dark:border-slate-700/50 dark:bg-slate-950/40 sm:grid-cols-2">
              <div className="min-w-0">
                <p className="text-xs font-medium uppercase text-slate-400">{copy.loginUrl}</p>
                <p className="truncate font-medium text-slate-700 dark:text-slate-200">{appOrigin ? `${appOrigin}/login` : '/login'}</p>
              </div>
              <div className="min-w-0">
                <p className="text-xs font-medium uppercase text-slate-400">{copy.email}</p>
                <p className="truncate font-medium text-slate-700 dark:text-slate-200">{createdUser.email}</p>
              </div>
              <div className="min-w-0">
                <p className="text-xs font-medium uppercase text-slate-400">{adminCopy.common.role}</p>
                <p className="truncate font-medium text-slate-700 dark:text-slate-200">{createdUser.role === 'ADMIN' ? adminCopy.common.admin : adminCopy.common.user}</p>
              </div>
              <div className="min-w-0">
                <p className="text-xs font-medium uppercase text-slate-400">{adminCopy.common.package}</p>
                <p className="truncate font-medium text-slate-700 dark:text-slate-200">{createdUser.packageName || copy.noPackage}</p>
              </div>
            ...
            <textarea
              readOnly
              value={formatUserDetails(createdUser, appOrigin || '', copy, adminCopy.common)}
