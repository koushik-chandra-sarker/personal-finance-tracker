...
import { Plus, Trash2, Search, ArrowLeftRight, TrendingUp, TrendingDown } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl border border-slate-600/50 bg-slate-800/50 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-indigo-500"
          />
        </div>
        <select
          value={typeFilter}
          onChange={(e) => setTypeFilter(e.target.value)}
          className="rounded-xl border border-slate-600/50 bg-slate-800/50 px-4 py-2.5 text-sm text-white focus:outline-none focus:border-indigo-500"
        >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-2">
          {filtered.map((tx) => (
            <div key={tx.id} className="flex items-center gap-3 p-4 rounded-2xl border border-slate-700/50 bg-slate-800/50 hover:bg-slate-800/70 transition-all group">
          ...
          {Array.from({ length: pages }, (_, i) => (
            <button
              ...
              className={`w-9 h-9 rounded-lg text-sm font-medium transition-colors ${
                currentPage === i + 1 ? 'bg-indigo-500 text-white' : 'text-slate-400 hover:bg-white/5'
              }`}
            >
              {i + 1}
            </button>
